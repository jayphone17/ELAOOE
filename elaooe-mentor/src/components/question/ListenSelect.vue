    ...
    <v-card-text>
      ...
      <div>
        <ol>
          ...
          >
            {{ field.answer }}
            {{ ownSchema.content.answer === field.answer ? "✅" : "❌" }}
          </li>
        </ol>
      </div>
    </v-card-text>
    ...
    <v-expand-transition>
      <v-card v-if="reveal" class="transition-fast-in-fast-out v-card--reveal">
        <v-card-text class="pb-0">
          ...
          <v-row
            v-for="(field, i) in ownSchema.content.fields"
            :key="field.id"
            no-gutters
          >
            <v-col
            ...
            ></v-col>
            <v-col md="auto"
              ><v-btn @click="removeField(field.id)">删除</v-btn></v-col
            >
          </v-row>
          <v-btn @click="addField" color="blue" class="ma-2 white--text" block>
            添加新选项
          </v-btn>
          <v-row no-gutters>
            <v-col>
              <v-select
                v-model="ownSchema.content.answer"
                :items="ownSchema.content.fields"
                item-text="answer"
                item-value="answer"
                label="答案"
                persistent-hint
